// lab1_rwlock_unsorted.c — CS4532 Lab 1: Parallel (global read–write lock, unsorted list, readable output + avg/std)
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
#include <math.h>
...
// Thread counts to test (as in the lab table)
...
/* Timing helper: milliseconds using clock() (thread creation, init excluded) */
static double now_ms(void) {
    return (double)clock() * 1000.0 / CLOCKS_PER_SEC;
}

/* ==========================================================
   RW-lock wrappers (global read–write lock)
   ========================================================== */
static bool Member_rd(struct Node** headp, int value, pthread_rwlock_t* rw) {
    pthread_rwlock_rdlock(rw);
    bool r = Member_serial(*headp, value);
    pthread_rwlock_unlock(rw);
    return r;
}
static bool Insert_wr(struct Node** headp, int value, pthread_rwlock_t* rw) {
    pthread_rwlock_wrlock(rw);
    bool r = Insert_serial(headp, value);
    pthread_rwlock_unlock(rw);
    return r;
}
static bool Delete_wr(struct Node** headp, int value, pthread_rwlock_t* rw) {
    pthread_rwlock_wrlock(rw);
...
/* ==========================================================
   Worker threads (one global RW-lock guards the whole list)
   ========================================================== */
typedef struct {
    // Shared:
    struct Node** headp;
...
static void* worker_fn(void* arg) {
    ...
    // Synchronize start so we time only the M ops window
    ...
    for (int i = a->start_index; i < a->end_index; ++i) {
        ...
        if (t == OP_MEMBER) {
            int key = rand_key();
            if (Member_rd(a->headp, key, a->rwlock)) a->member_hits++;
        } else if (t == OP_INSERT) {
            // Keep drawing until we get a key that's not present (checked under rdlock);
            // uniqueness is ultimately enforced by Insert_serial inside wrlock.
            int key = rand_key();
            while (Member_rd(a->headp, key, a->rwlock)) key = rand_key();
            if (Insert_wr(a->headp, key, a->rwlock)) a->insert_ok++;
        } else { // OP_DELETE
            int key = rand_key();
            if (Delete_wr(a->headp, key, a->rwlock)) a->delete_ok++;
        }
    }
    return NULL;
}
    ...
    for (int ci = 0; ci < NUM_CASES; ++ci) {
        ...
        for (int ti = 0; ti < N_THREADS; ++ti) {
            ...
            for (int r = 0; r < REPS; ++r) {
                ...
                // 3) Create RW-lock and barrier; spawn T workers
                pthread_rwlock_t rwlock;
                ...
                // Partition M ops among T threads
                int base = 0;
                for (int t = 0; t < T; ++t) {
                    int len = M / T + (t < (M % T) ? 1 : 0);
                    args[t].headp = &head;
                    args[t].rwlock = &rwlock;
                    args[t].ops = ops;
                ...
                printf("%-8s %-6d %-12.3f %-12lu %-12lu %-12lu\n",
                       "rwlock", r + 1, elapsed, mh, ins_ok, del_ok);
            ...
            printf("-----------------------------------------------------\n");
            printf("Summary (rwlock, T=%d): Average = %.3f ms, Std = %.3f ms over %d runs\n\n",
                   T, mean, std, REPS);
        }
    }
